...
An active wound (pressure ulcer, diabetic foot ulcer, venous ulcer, etc.)
Active Medicare Part B coverage
Documented wound measurements (length, width, depth) and drainage level
...
Full endpoint documentation is in API.md. The short version:
...
Rate limiting: Every request has a 30% chance of returning HTTP 429. The response includes a Retry-After header. You must implement retry logic — pipelines that don't handle 429s will fail to load data. See API.md for recommended retry patterns.

What to Build
Required
1. Data ingestion pipeline Fetch all patients, diagnoses, coverage, notes, and assessments from the API. Handle rate limiting. Store the results somewhere queryable (a local database, dataframe, files — your choice).

2. Wound data extraction From each progress note and assessment, extract:
...
3. Eligibility output table Produce one row per patient with:
...
4. Presentation Walk us through your output as if presenting to a non-technical biller. What do they see? How do they know what to act on?

5. Visual output Display your results in a visual format — a dashboard, UI, or interactive table. A biller should be able to see patient routing decisions at a glance without reading raw data.
